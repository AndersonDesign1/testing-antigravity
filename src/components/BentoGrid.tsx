...
const BentoCard = ({ title, description, imageSrc, className, children }: { title: string, description: string, imageSrc?: string, className?: string, children?: React.ReactNode }) => (
  <div className={`relative border border-gaming-white/20 bg-gaming-black p-0 flex flex-col justify-end overflow-hidden group transition-all duration-500 hover:border-gaming-white/60 ${className}`}>
    ...
    <div className="z-10 relative p-8 border-t border-gaming-white/10 bg-gaming-black/40 backdrop-blur-sm translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
      <h3 className="text-3xl font-black mb-2 uppercase tracking-tighter text-gaming-white">{title}</h3>
      <p className="font-mono text-xs tracking-widest text-gaming-white/70 uppercase border-l-2 border-gaming-brown pl-3">{description}</p>
      {children}
    </div>
  </div>
);
  ...
  return (
    <section className="w-full py-32 px-4 bg-gaming-black text-gaming-white relative">
      <div className="max-w-7xl mx-auto mb-12 border-b border-gaming-white/20 pb-4 flex justify-between items-end">
         <h2 className="text-sm font-mono tracking-[0.3em] opacity-50">CORE MODULES</h2>
